...
class GroupView(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(request_body=openapi.Schema(type=openapi.TYPE_OBJECT, properties={"latitude": openapi.Schema(type=openapi.TYPE_NUMBER), "longitude": openapi.Schema(type=openapi.TYPE_NUMBER)}), responses={200: GroupSerializer})
    def list(self, request):
        ...
        groups = get_nearby_group_within(latitude=float(latitude), longitude=float(longitude), km=radius, limit=number_of_groups_to_return)

        groups_data = NearbyGroupSerializer(groups, many=True)
        return Response(groups_data.data)

    @swagger_auto_schema(request_body=openapi.Schema(type=openapi.TYPE_OBJECT, properties={"group_id": openapi.Schema(type=openapi.TYPE_INTEGER)}), responses={200: GroupSerializer})
...
class ListGroupView(GenericAPIView):
    ...
    @swagger_auto_schema(request_body=openapi.Schema(type=openapi.TYPE_OBJECT, properties={"page": openapi.Schema(type=openapi.TYPE_INTEGER)}), responses={200: GroupSerializer})
    def post(self, request):
